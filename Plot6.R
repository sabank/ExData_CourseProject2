plot6 <- function(){
    ...
    ## total emission by year in Baltimore City and Los Angeles County from motor vehicle sources
    x <- nei %>% filter(SCC %in% code & (fips == "24510" | fips == "06037")) %>%
        group_by(fips, year) %>% summarise(PM25 = log(sum(Emissions)))
    ...
    # draw plot
    gp <- g + geom_line(aes(x=year, y=PM25, color = fips)) +
        labs(title = "Baltimore City vs Los Angeles County: Total Emission \n Motor vehicle sources") +
        labs(y = expression("log" * PM[2.5])) +
        scale_color_discrete(name = "",                                  # remove legend title
                            breaks = c("06037", "24510"),                # modify original legend labels
                            labels = c("L.A. County", "Baltimore City")) # by new text
    print(gp)
