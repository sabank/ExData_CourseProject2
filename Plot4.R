plot4 <- function(){
    ...
    ## logical vector searching for string 'Coal'
    logicalVector <- with(scc, grepl("Coal", scc[,4]))
    extractrow <- which(logicalVector)
    code <- scc[extractrow,1]
    
    ## total emission by year from coal combustion-related sources
    x <- nei %>% filter(SCC %in% code) %>% group_by(year) %>% summarise(PM25 = log(sum(Emissions)))
